...
module.exports = async function(changes, service, app, force) {
  ...
  for(let change of changes) {
    ...
    switch(change.operation) {
      ...
      case "createProperty": {
        ...
        await dao.get(['database', 'query', 'query-get.test', `(${
            async (input, output, { table, update }) => 
                await input.log(table).onChange((obj, oldObj) => {
                  if(obj) output.table(table).update(obj.id, [{ op: 'merge', update }])
                })
        })`, { table, update }])
      } break;
      case "renameProperty": {
