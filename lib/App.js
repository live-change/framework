...
class App {
  ...
  async updateService( service, { path, updaters, force } = {}) {
    ...
    let oldServiceJson = await this.dao.get(['database', 'get', this.databaseName, 'services', service.name])
    if(!oldServiceJson) {
  ...
  async triggerService(service, data) {
    if(!data.id) data.id = this.app.generateUid()
    const triggersTable = this.app.splitCommands ? `${this.name}_triggers` : 'triggers'
    const objectObservable = this.dao.observable(['database', 'tableObject', this.databaseName, triggersTable, id],
  ...
  async command(service, data) {
    if(!data.id) data.id = this.app.generateUid()
    const commandsTable = this.app.splitCommands ? `${this.name}_commands` : 'commands'
    const objectObservable = this.dao.observable(['database', 'tableObject', this.databaseName, commandsTable, id],
        ReactiveDao.ObservableValue)
