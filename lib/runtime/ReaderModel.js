        ...
        async (input, output, { tableName, indexName, range, options }) => {
          ...
          await (await input.index(indexName)).range(range).onChange(async (obj, oldObj) => {
            ...
            if(obj && obj.to) {
              ...
              const ind = obj
              const observer = await reader.onChange(async obj => {
                const data = obj && objectMapper(obj, ind)
                const oldData = objectStates.get(ind.to) || null
                output.change(data, oldData)
                if(data) {
                  objectStates.set(ind.to, data)
                } else if(oldObj) {
                  objectStates.delete(ind.to)
                }
              })
              ...
              const oldData = oldObj && oldObj.to && objectStates.get(oldObj.to)
              output.change(null, oldData)
